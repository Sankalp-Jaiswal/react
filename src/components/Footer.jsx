...
const Footer = () =>(
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className='flex-1 mr-[200px] flex flex-col justify-start'>
        <img src={logo} alt="logo" className="w-[266px] h-[72px] object-contain" />
        <p className={`${styles.paragraph} mt-4 min-w-[250px] max-w-[310px]`}>
          ...
          <div key={footerLink.id} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
            ...
            <ul className='list-none mt-4'>
              {footerLink.links.map((link,index)=>(
                <li key={link.name} className={`font-poppins font-normaltext-[160px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index!== footerLink.links.length-1? 'mb-4':'mb-0'}`}>
                  {link.name}
                </li>
              ))}
            </ul>
    ...
    <div className='w-full flex justify-between item-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]'>
      <p className='font-poppins font-normal text-[18px] text-center leading-[27px] text-white'>
        2024 Sankalp. All Rights Reserved.
      </p>
